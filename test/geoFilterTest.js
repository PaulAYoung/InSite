...
describe("geoFilter", function(){
    it("should not return any testData for zero max distance", function(){
        assert.equal(0, geoFilter([1, 1], testData, 0).length);
    });

    it("should only return item 7", function(){
        assert.equal(7, geoFilter([-122.32779622077942, 37.89016347485787], testData, 1)[0].id);
    });

    it("should work with L.LatLong objects", function(){
        assert.equal(7, geoFilter(L.latLng([-122.32779622077942, 37.89016347485787]), testData, 1)[0].id);
    });
});
